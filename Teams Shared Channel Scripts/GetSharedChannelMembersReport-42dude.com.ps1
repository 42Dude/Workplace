...
FORKED BY 42DUDE.COM 10-7-2023. Added GroupID to the reports :)
============================================================================================
#>
param(
[string]$UserName, 
...
Function Process_TeamChannel
{
 Get-TeamAllChannel -GroupId $TeamId -MembershipType Shared | foreach {
  ...
  $Result = @{'GroupID'=$TeamId ;'Team Name'=$Teamname;'Shared Channel Name'=$ChannelName;'Shared Channel Type'=$SharedChannelType;'Description'=$Description;'Channel Members'=$AllMembers;'Members Count'=$MembersCount }
  $Results = New-Object PSObject -Property $Result
  $Results |select-object 'GroupID','Team Name','Shared Channel Name','Shared Channel Type','Description','Members Count','Channel Members' | Export-Csv -Path $OutputCSVName -Notype -Append
  ...
  foreach($Member in $Membership)
  {
   ...
   $Email=$Member.User
   $Role=$Member.Role
   $Output=@{'GroupID'=$TeamId;'Team Name'=$Teamname;'Shared Channel Name'=$ChannelName;'Shared Channel Type'=$SharedChannelType;'Description'=$Description;'Member Name'=$Name;'Member Email'=$Email;'Role'=$Role}
   $Outputs = New-Object PSObject -Property $Output
   $Outputs | Select-Object 'GroupID','Team Name','Shared Channel Name','Shared Channel Type','Description','Member Name','Member Email','Role' | Export-Csv -Path $OutputCSV -Notype -Append
  }
 }
}
